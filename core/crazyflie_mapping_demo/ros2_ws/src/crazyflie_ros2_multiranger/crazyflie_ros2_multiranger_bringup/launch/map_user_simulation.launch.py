...
def generate_launch_description():
    ...
    crazyflie_vel_mux = Node(
        ...
        parameters=[
            ...
            {'robot_prefix': '/crazyflie_user_sim'},
        ]
    )

    # ── Shared mapper ─────────────────────────────────────────────────────────
    # map_file is forwarded here.  When non-empty the mapper loads the saved
    # map and publishes it immediately; new scans are merged on top.
    # When empty it falls back to the normal origin-averaging startup.
    shared_mapper = Node(
        ...
        output='screen',
        parameters=[
            {'robot_prefixes': ['/crazyflie_user_sim']},
            {'map_file': map_file},
            {'use_sim_time': False},
        ]
    )
