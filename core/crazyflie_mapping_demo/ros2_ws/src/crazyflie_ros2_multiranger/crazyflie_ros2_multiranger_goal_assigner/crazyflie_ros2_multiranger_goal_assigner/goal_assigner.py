...
FRONTIER_STEP     = 1
MIN_CLUSTER_SIZE  = 3
MIN_UNKNOWN_CELLS = 2     # cluster must border at least this many unknown cells
...
class GoalAssigner(Node):
    ...
    def _status_cb(self, msg: String, prefix: str):
        ...
        if status in ('REACHED', 'FAILED'):
            ...
            self.get_logger().info(
                f'[{prefix}] recalled — removing from assignment pool.')
            ds.status = 'RECALLED'
            ds.goal   = None
    ...
    def _do_assignments(self, clusters: list, now: float) -> bool:
        ...
        for prefix, ds in self.drones.items():
            # Recalled drones are permanently excluded from assignment
                ...
                for dr in range(-1, 2):
                    for dc in range(-1, 2):
                        ...
                        if 0 <= nr < H and 0 <= nc < W:
                            if data[nr*W+nc] == -1:
                                total_unk += 1
            if total_unk < MIN_UNKNOWN_CELLS:
                continue
