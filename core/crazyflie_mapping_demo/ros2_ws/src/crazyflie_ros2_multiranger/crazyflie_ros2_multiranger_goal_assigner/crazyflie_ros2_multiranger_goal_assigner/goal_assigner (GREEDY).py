...
per drone using information value, and assigns non-overlapping goals using
greedy allocation. Publishes one goal per drone on /cfX/assigned_goal
(geometry_msgs/Point). Listens to /cfX/goal_status (std_msgs/String:
"REACHED" or "FAILED") so it only reassigns on completion or genuine
failure — never on a scoring fluctuation.

Termination mirrors the original frontier_exploration_multiranger.py logic:

  Condition 1 — zero clusters 3 times in a row:
    Triggers a BFS reachability check. If BFS confirms no unknown cells
    are reachable from any drone, sends all drones home. If BFS still
    finds reachable unknowns, resets the counter and keeps waiting.

  Condition 2 — BFS reachability check (3s cooldown):
    Called whenever zero_cluster_count >= 3. BFS runs from every drone
    position. If none can reach any unknown cell, exploration is done.

  Condition 3 — all drones FAILED with no assignable frontiers remaining:
    If every drone reports FAILED and the assigner cannot assign any
    available cluster, exploration is done.
...
class GoalAssigner(Node):
    ...
    def _do_assignments(self, clusters: list, now: float) -> bool:
        ...
        for prefix in drones_needing_goal:
            ds     = self.drones[prefix]
            px, py = ds.position
            ...
            for wx, wy, size, unknown, score in clusters:
                if any(math.hypot(wx - tx, wy - ty) < 0.5
                    ...
                    continue
                if math.hypot(wx - px, wy - py) < MIN_FRONTIER_DIST:
                    continue
                if score > best_score:
                    best_score = score
                    best_goal  = (wx, wy)
            ...
            if best_goal is None:
                self.get_logger().warn(
                    f'[{prefix}] no uncontested frontier available.')
                continue

            ds.goal              = best_goal
            ds.status            = 'NAVIGATING'
            ds.goal_cluster_size = best_size
            taken.add(best_goal)
            assigned_any = True

            msg   = Point()
            msg.x = float(best_goal[0])
            msg.y = float(best_goal[1])
            msg.z = 0.0
            self.goal_pubs[prefix].publish(msg)
            self.get_logger().info(
                f'[{prefix}] assigned → ({best_goal[0]:.2f},{best_goal[1]:.2f}) '
                f'score={best_score:.2f} size={best_size}')
